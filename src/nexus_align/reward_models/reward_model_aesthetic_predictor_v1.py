"""AestheticPredictorV1 reward model for evaluating image generation."""
...
class AestheticPredictorV1(BaseRewardModel):
    """
    AestheticPredictorV1 reward model for evaluating image generation.

    References:
        - Official repo: https://github.com/LAION-AI/aesthetic-predictor.
        - Used repo: https://github.com/shunk031/simple-aesthetics-predictor.
        - Checkpoint: https://huggingface.co/shunk031/aesthetics-predictor-v1-vit-large-patch14.
    """

    def __init__(self, device: torch.device, kwargs: dict) -> None:
        super().__init__("AestheticPredictorV1", device, kwargs)
    ...
    def load_model(self) -> tuple[torch.nn.Module, torch.nn.Module]:
        ...
        model = AestheticsPredictorV1.from_pretrained(self.model_path)
